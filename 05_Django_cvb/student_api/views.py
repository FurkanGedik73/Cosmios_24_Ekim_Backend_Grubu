...
class StudentCV(ListCreateAPIView):
    queryset = Student.objects.filter(age=18)  #yaşına göre filtreleme yapıyoruz.
    serializer_class = StudentSerializer
    
    def get_queryset(self):
        if self.request.user.is_staff:       #eğer admin rolune sahip biri değilse kontrol edin
            return Student.objects.all()        # yukarıda yazılı olan filtre devreye girsin
        return Student.objects.filter(age=18)          #eğer admin değil ifade bütün değerleri görüntülesin
...
class StudentDetailCV(RetrieveUpdateDestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        instance = self.get_object()            #instance ilgilendiğimiz obje
